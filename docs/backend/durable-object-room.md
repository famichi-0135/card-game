...
Worker 統合テストでは、Google OAuth で作成されたものと同じ Better Auth セッションを2人分用意し、異なる陣営のスターターデッキで招待対戦を開始する。Google の実アカウントを使う認可・callback は staging で手動確認する。双方のWebSocket接続、相手操作による更新通知とHTTP再同期、切断・再接続時のプレゼンス通知を確認する。期限超過による敗北・引き分けの状態遷移は、実時間の待機を避けるためゲームエンジンの単体テストで決定的に確認する。
