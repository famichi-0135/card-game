...
`receivedAt`はクライアントから受け取らない。対戦単位でコマンドを直列化し、同じ認証済みプレイヤーから完全に同じ内容の`commandId`が再送された場合だけ、保存済みの最初の結果を返す。別プレイヤーまたは異なる内容によるID再利用は競合として拒否し、保存済みの`PlayerGameView`を返さない。
...
WebSocketは認証済み参加者だけが接続でき、クライアントからサーバーへのゲーム操作には使用しない。`GameRealtimeUpdate`は`gameId`、`stateVersion`、`latestEventSequence`だけを含む更新通知であり、`PlayerGameView`、カード情報、公開イベント本体を含めない。受信側は通知の内容を状態として適用せず、HTTPスナップショットを再取得する。接続の切断・不正メッセージ・再接続失敗時にも、HTTPコマンド送信とスナップショット取得は継続して利用できる。

## 公開カードカタログ

`PlayerGameView.cardCatalogVersion`に対応する公開カードカタログを次の HTTP アダプターで返す。内部`CardCatalog`を直接シリアライズしてはならない。

| 操作             | エンドポイント                               | 成功時の応答                     |
| ---------------- | -------------------------------------------- | -------------------------------- |
| 公開カタログ取得 | `GET /api/card-catalogs/:cardCatalogVersion` | `{ catalog: PublicCardCatalog }` |

カタログは表示・操作候補判定に必要なカード名、属性、種別、コスト、基礎攻撃力、継続期間、公開ルール文章、画像識別子、連鎖条件、効果対象選択情報だけを含める。`handlerId`、カスタム効果`config`、未公開条件、乱数seedは含めない。現行版は常に取得可能とし、進行中のゲーム、または終了から24時間の再接続猶予中のゲームが参照する過去版は必ず保持する。現行版でも保持版でもないカタログは`404 CARD_CATALOG_NOT_FOUND`で返す。保持の詳細は[ゲームセッション Durable Object](./durable-object-room.md#カタログと再接続の保持)を参照する。

このエンドポイントは公開情報だけを返すため認証不要とする。ただし、対戦画面は認証済みスナップショットで得た`cardCatalogVersion`だけを要求し、ゲーム開始時に固定された版と異なるカタログでコマンド候補を作らない。
