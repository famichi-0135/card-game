...
`FORFEIT_GAME`は同じ`POST /api/games/:gameId/commands`へ送る明示的な対戦中止コマンドである。進行中の認証済み参加者だけが送信でき、送信者を敗北、相手を勝利として終了する。このコマンドは`phaseSequence`、`clientStateVersion`、フェーズ期限の不一致では拒否しないが、認証済みプレイヤー照合、参加者照合、`commandId`の冪等性は通常どおり適用する。
