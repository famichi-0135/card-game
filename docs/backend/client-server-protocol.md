...
カタログは表示・操作候補判定に必要なカード名、属性、種別、コスト、基礎攻撃力、継続期間、公開ルール文章、画像識別子、連鎖条件、効果対象選択情報だけを含める。`handlerId`、カスタム効果`config`、未公開条件、乱数seedは含めない。存在しない、または保持期限を過ぎた版は`404 CARD_CATALOG_NOT_FOUND`で返す。
