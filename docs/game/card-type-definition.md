...
すべてのカード定義は`Faction`を持ち、`disaster`または`countermeasure`のどちらか一方に所属する。効果対象の`self`・`opponent`はプレイヤー基準で解決し、異なる陣営のカードを同じデッキへ入れることはデッキ検証で拒否する。
...
この節の例は、ゲームエンジンが受理する複合効果の形式を説明するための仮のカード定義であり、現行の初期カードカタログに含まれるカードではない。
...
最大ラウンド判定で総パワーを比較する場合は、手順2で確定した値を使用する。効果削除後の盤面から総パワーを再計算してはならない。
...
この例は複合効果の形式を示すだけであり、現行スターターデッキのカード定義ではない。
...
### 49.1 現行スターターデッキのサポートカード

現行カタログ`initial-catalog-v5-learning-content`では、各陣営のスターターデッキを、みなもと12枚（各属性4枚）、攻撃12枚、サポート6枚で構成する。攻撃カードは各属性にコスト・基本攻撃力が`1 / 2 / 3`となる連鎖軸を持つ。通常カードのコスト上限は3とする。

みなもとカードの表示名は、災害側を`大地のみなもと`、`水のみなもと`、`空のみなもと`、対策側を`備える力のみなもと`、`守る力のみなもと`、`つながる力のみなもと`とする。攻撃・サポートカードの表示名と学習解説は、防災素材に対応付ける。カード詳細では学習解説の後に`ゲーム上の効果`を併記し、内部ID、属性、コスト、基本攻撃力、連鎖、効果定義は変更しない。

サポートカードは各1枚であり、以下の効果を持つ。災害側と対策側は同じ役割・コスト分布とし、スタミナ操作だけは災害側が相手へ`-1`、対策側が自分へ`+1`を適用する。

| 定義IDの末尾             | コスト | 効果                                                         |
| ------------------------ | -----: | ------------------------------------------------------------ |
| `support-group-boost`    |      1 | 自分の攻撃グループの攻撃力をラウンド終了まで`+1`             |
| `support-remove-support` |      1 | 相手のサポートカードを1枚除去                                |
| `support-reduce-mana`    |      2 | 相手の選択した属性のみなもとを1減少                          |
| `support-stamina`        |      1 | 災害側は相手のスタミナを1減少、対策側は自分のスタミナを1回復 |
| `support-remove-group`   |      3 | 相手の攻撃グループを1つ除去                                  |
| `support-destroy-draw`   |      1 | 自分がカードを1枚引く。対象指定や攻撃グループ除去は行わない  |
...
攻撃カードの`chainableCardIds`は同陣営の攻撃カードだけを参照できる。異なる陣営への参照はカタログ読み込み時に拒否する。
...
カード定義の`faction`は必須とし、`disaster`または`countermeasure`以外の値を拒否する。初期カタログは両陣営のカードを収録し、それぞれ独立した30枚スターターデッキが構築可能でなければならない。
...
- `chainableCardIds`の参照先が効果元カードと同じ陣営である
