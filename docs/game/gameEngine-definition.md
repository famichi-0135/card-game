...
`GameRules`はコンテキスト構築時に検証する。初期エンジンでは`playerCount === 2`、枚数・スタミナ・グループ数・ラウンド数・時間が0以上の安全な整数、最小値が対応する最大値以下、デッキ種別条件を同時に満たせることを必須とする。具体的には、みなもとカードと攻撃カードの最小枚数合計がデッキ枚数以下であり、みなもと・攻撃・サポートカードの最大枚数合計がデッキ枚数以上でなければならない。検証済みルールは対戦中に変更しない。
...
`commandId`は1〜128文字とする。同じ`commandId`を持つコマンドを再度受信した場合は、ゲーム状態を再変更しない。
...
`status === "active"`の場合、`phase`は`initializing`と`finished`以外の進行フェーズとする。`initializing`状態は初期化処理内だけで使用し、永続状態の検証では拒否する。
...
- `stateVersion`は0以上、`phaseSequence`、`nextEffectSequence`、`nextEventSequence`は1以上の安全な整数
