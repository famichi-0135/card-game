...
1. 実装済み: Google OAuthと匿名ゲストの認証画面、対戦・待機部屋 URL からの同一オリジン内戻り先制御を実装する。匿名ゲストをGoogleへ引き継ぐ場合も、ゲーム用PlayerIdを維持する。
2. 実装済み: トップ、ロール選択、スターターデッキ生成、招待部屋の作成・参加・取消、準備完了後の対戦開始、2秒ポーリング、対戦開始時の遷移を実装する。初期ローンチのロールごとのデッキは固定する。
3. 受け入れ: 認証済みの2人が異なるロールを選び、招待 URL を経由して対戦開始まで到達できることを Worker 統合環境で確認する。

Worker 統合テストで、Better Auth の2ユーザーによるスターターデッキ作成、招待、対戦開始、双方のWebSocket接続、相手操作の更新通知、切断・再接続時のプレゼンス通知までを確認済みとする。期限超過による切断敗北はゲームエンジンの決定的な単体テストで検証する。

防災情報画面と保存済みデッキの編集は初期ローンチ外とする。
