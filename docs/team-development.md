...
3. `Quality` とレビューの条件を満たしたら `Squash and merge` する。
...
必要な承認、コメント解決、`Quality` の完了後に、PR 作成者が `Squash and merge` でマージします。マージ後、GitHub 上の作業ブランチは自動削除されます。各自のローカルブランチは、次の手順で整理します。
