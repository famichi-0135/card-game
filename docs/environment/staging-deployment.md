...
`/api/auth/callback/google`はGoogleから認可結果を受け取るサーバー用エンドポイントであり、画面ではない。ブラウザで直接開くとOAuth stateがないため失敗する。利用者はstagingアプリの`/login`からGoogleログインを開始する。

公開設定と Secret は、次のように分けて管理する。
...
`BETTER_AUTH_URL`、`BETTER_AUTH_TRUSTED_ORIGINS`、`GOOGLE_CLIENT_ID`は、`apps/backend/wrangler.jsonc` の`env.staging.vars`で管理する。これらの変更はPRでレビューし、Backendを再デプロイして反映する。

`BETTER_AUTH_SECRET`と`GOOGLE_CLIENT_SECRET`は、Git管理外の`apps/backend/.env.staging.local`にだけ保持し、Cloudflare Secretへ同期する。値はリポジトリ、PR、Issue、ログ、チャットへ追加しない。

```sh
cd apps/backend
pnpm exec wrangler secret bulk --env staging .env.staging.local
```

Secretの更新は直ちにWorkerへ反映される。公開設定を変更した場合だけ、続けてBackendを再デプロイする。
