...
Squash mergeでは作業ブランチのcommit自体は`main`の祖先にならないため、`git branch -d`は正常にマージ済みでも失敗します。`git branch -D`を実行する前に、GitHub上でPRが`Merged`になっていること、`git status`に未コミット変更がないこと、PRへ含めなかった作業がブランチに残っていないことを確認します。
